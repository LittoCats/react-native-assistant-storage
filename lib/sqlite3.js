import md5 from './md5'
import Assistant, {sqlite3} from './assistant'

// function sqlite3.open(dbPath, callback)
// function callback(error, dbhandle)

// function sqlite3.execute(dbhandle, sql, paramsArray, callback)
// function callback(error[, dataArray])

// function sqlite3.close(dbhandle, callback)
...
export default class Connection {
  ...
  open() {
    ...
    return new Promise((resolve, reject)=>{
      sqlite3.open(this.__file__, (error, handle)=>{
        if (error) {
  ...
  execute(sql, ...params) {
    return this.open().then((conn)=> {
      return new Promise((resolve, reject)=>{
        sqlite3.execute(this.__db__, sql, params, (error, result)=> {
          if (error) {
            var err = new Error(error.msg);
            err.code = error.code;
            reject(err);
  ...
  close() {
    ...
    return new Promise((resolve, reject)=>{
      sqlite3.close(this.__db__, function (error) {
        if (error) {
          var err = new Error(error.msg);
          err.code = error.code;
          reject(err);
