...
function config(persistence, dialect) {
  ...
  persistence.schemaSync = function(tx, callback, emulate) {
    ...
    if (callback === null) {
      ...
      return new Promise(function(resolve, reject){
        session.schemaSync(tx, function(tx, err){
    ...
    var queries = [],
      meta, colDefs, otherMeta, tableName;
        ...
        for (var rel in meta.hasMany) {
          if (meta.hasMany.hasOwnProperty(rel) && meta.hasMany[rel].manyToMany) {
            ...
            if (!persistence.generatedTables[tableName]) {
              ...
              if (otherMeta.hasMany[inv].type.meta != meta) continue;
              ...
              if (meta.isMixin) columns.push([p1 + "_class", tm.classNameType])
              if (otherMeta.isMixin) columns.push([p2 + "_class", tm.classNameType])
  ...
  persistence.flush = function(tx, callback) {
    ...
    if (callback === null) {
      return new Promise(function(resolve, reject){
        session.flush(tx, function(results, err){
  ...
  persistence.reset = function(tx, callback) {
    ...
    if (callback === null) {
      return new Promise(function(resolve, reject){
        session.reset(tx, function(results, err){
          err ? reject(err) : resolve(results);
        });
      });
    }
  ...
  persistence.AndFilter.prototype.sql = function(meta, alias, values) {
    return "(" + this.left.sql(meta, alias, values) + " AND " + this.right.sql(meta, alias, values) + ")";
  };
  persistence.OrFilter.prototype.sql = function(meta, alias, values) {
    return "(" + this.left.sql(meta, alias, values) + " OR " + this.right.sql(meta, alias, values) + ")";
  };
  ...
  persistence.DbQueryCollection.prototype.list = function(tx, callback) {
    ...
    function selectAll(meta, tableAlias, prefix) {
      ...
      for (var p in meta.fields) {
        if (meta.fields.hasOwnProperty(p)) {
          selectFields.push(tm.inVar("`" + tableAlias + "`.`" + p + "`", meta.fields[p]) + " AS `" + prefix + p + "`");
        }
      }
      for (var p in meta.hasOne) {
        if (meta.hasOne.hasOwnProperty(p)) {
          selectFields.push(tm.inIdVar("`" + tableAlias + "`.`" + p + "`") + " AS `" + prefix + p + "`");
        }
      }
      return selectFields;
    }
    ...
    for (var i = 0; i < this._prefetchFields.length; i++) {
      ...
      if (thisMeta.isMixin) throw new Error("cannot prefetch a mixin");
      ...
      joinSql += "LEFT JOIN `" + thisMeta.name + "` AS `" + tableAlias + "` ON `" + tableAlias + "`.`id` = `" + PrefetchFrom + '`.`' + prefetchField + "` ";
    }
    var whereSql = "WHERE " + [this._filter.sql(meta, mainAlias, args)].concat(additionalWhereSqls).join(' AND ');
    var sql = "SELECT " + selectFields.join(", ") + " FROM `" + entityName + "` AS `" + mainAlias + "` " + joinSql + " " + whereSql;
    if (this._additionalGroupSqls.length > 0) {
      sql += this._additionalGroupSqls.join(' ');
    }
    if (this._orderColumns.length > 0) {
      sql += " ORDER BY " + this._orderColumns.map(function(c) {
        return (c[2] ? "`" : "LOWER(`") + mainPrefix + c[0] + (c[2] ? "` " : "`) ") + (c[1] ? "ASC" : "DESC");
      }).join(", ");
    }
    if (this._limit >= 0) {
  ...
  persistence.DbQueryCollection.prototype.destroyAll = function(tx, callback) {
    ...
    joinSql += this._additionalJoinSqls.join(' ');
    var args = [];
    var whereSql = "WHERE " + [this._filter.sql(meta, null, args)].concat(additionalWhereSqls).join(' AND ');
  ...
  persistence.ManyToManyDbQueryCollection.prototype.persistQueries = function() {
    ...
    for (var i = 0; i < this._localRemoved.length; i++) {
      queries.push(["DELETE FROM  " + rel.tableName + " WHERE `" + direct + "_" + this._coll + "` = " + tm.outIdVar("?") + " AND `" + inverse + '_' + rel.inverseProperty + "` = " + tm.outIdVar("?"), [tm.entityIdToDbId(this._obj.id), tm.entityIdToDbId(this._localRemoved[i].id)]]);
    }
    this._localRemoved = [];
    return queries;
  };
